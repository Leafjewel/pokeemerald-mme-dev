...
#include "trig.h"
#include "gpu_regs.h"
#include "field_camera.h"	//snow weather fix

#define DROUGHT_COLOR_INDEX(color) ((((color) >> 1) & 0xF) | (((color) >> 2) & 0xF0) | (((color) >> 3) & 0xF00))

/*enum		//dynamic overworlds
{
    GAMMA_NONE,
    GAMMA_NORMAL,
    GAMMA_ALT,
...
//static const u8 sBasePaletteGammaTypes[32] =		//dynamic overworlds
...
void StartWeather(void)
{
    ...
    {
        //u8 index = AllocSpritePalette(0x1200);	//dynamic overworlds
        u8 index =15;
        CpuCopy32(gUnknown_083970E8, &gPlttBufferUnfaded[0x100 + index * 16], 32);
        BuildGammaShiftTables();
        gWeatherPtr->altGammaSpritePalIndex = index;
        //gWeatherPtr->weatherPicSpritePalIndex = AllocSpritePalette(0x1201);	//dynamic overworlds
...
static void Task_WeatherInit(u8 taskId)
{
    ...
    {
		UpdateCameraPanning();	//Snow Weather Fix
...
static void BuildGammaShiftTables(void)
{
    ...
    u16 v11;
    s16 dunno;
	u8 i;	//dynamic overworlds

    for (i = 0; i <= 12; i++)
        sBasePaletteGammaTypes[i] = GAMMA_NORMAL;	//end section
...
//void LoadCustomWeatherSpritePalette(const u16 *palette)	//dynamic overworlds
...
void UpdatePaletteGammaType(u8 index, u8 gammaType)		//dynamic overworlds, 3
{
    if (index != 0xFF)
