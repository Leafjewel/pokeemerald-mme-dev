...
#include "event_data.h"	//pokemart items by badge
...
static void Task_ReturnToItemListAfterTMShopPurchase(u8 taskId);	//tm shops
static void Task_ReturnToItemListAfterDecorationPurchase(u8 taskId);
static void Task_HandleShopMenuBuy(u8 taskId);
static void Task_HandleShopMenuSell(u8 taskId);
static void BuyMenuPrintItemDescriptionAndShowItemIcon(s32 item, bool8 onInit, struct ListMenu *list);
//static void BuyMenuPrintPriceInList(u8 windowId, s32 item, u8 y);	tm shops
static void BuyMenuPrintPriceInList(u8 windowId, s32 item, u8 y, u8 itemPos);
static u8 GetNumberOfBadges(void); //pokemart items by badge

//added no Pokedex shop
static const u16 sShopInventory_NoPokedex[] = {
    ITEM_POTION,
    ITEM_ANTIDOTE,
    ITEM_NONE
};
//inventory changed from generic Gen VIII to Emerald stock (with tweaks)
...
static u8 CreateShopMenu(u8 martType)
{
    ...
    //if (martType == MART_TYPE_NORMAL)		tm shops
...
{
    u16 i = 0;
	u8 badgeCount = GetNumberOfBadges(); //pokemart items by badge

    //gMartInfo.itemList = items;	//pokemart items by badge
	//if (items == NULL)			tweaked to add no Pokedex mart in
    //    gMartInfo.itemList = sShopInventories[badgeCount];
	if (items == NULL && !FlagGet(FLAG_ADVENTURE_STARTED))
        gMartInfo.itemList = sShopInventories[0];
	else if (items == NULL)
        gMartInfo.itemList = sShopInventories[badgeCount + 1];
    else
...
{
    //if (gMartInfo.martType == MART_TYPE_NORMAL)	tm shops
...
static void BuyMenuAddItemIcon(u16 item, u8 iconSlot)
{
    ...
    //if (gMartInfo.martType == MART_TYPE_NORMAL || item == 0xFFFF)		//tm shops
        ...
        default:
            ...
            else
            {
                ...
                else
                {
                    ...
                    //ConvertIntToDecimalStringN(gStringVar2, gShopDataPtr->totalCost, STR_CONV_MODE_LEFT_ALIGN, 6);	more money
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    else
    {
        if (gMain.newKeys & A_BUTTON)
        {
            ...
            //ConvertIntToDecimalStringN(gStringVar3, gShopDataPtr->totalCost, STR_CONV_MODE_LEFT_ALIGN, 6); more money
...
static void Task_ReturnToItemListAfterTMShopPurchase(u8 taskId)	//start tm shops section
{
    s16 *data = gTasks[taskId].data;
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    //PrintMoneyAmount(4, 38, 1, gShopDataPtr->totalCost, TEXT_SPEED_FF);	//more money
...
void CreatePokemartMenu(const u16 *itemsForSale)
{
	...
	SetShopId(0);	//tm shops
    ClearItemPurchases();
    SetShopMenuCallback(EnableBothScriptContexts);
}

void CreateTMShopMenu(const u16 *itemsForSale, u8 shopId)	//start tm shops section
{
