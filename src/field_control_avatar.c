...
void FieldClearPlayerInput(struct FieldInput *input)
{
    ...
    //input->input_field_1_0 = FALSE;	//register lr
    //input->input_field_1_1 = FALSE;	//register lr
	input->pressedLButton = FALSE;		//register lr
    input->pressedRButton = FALSE;		//register lr
...
void FieldGetPlayerInput(struct FieldInput *input, u16 newKeys, u16 heldKeys)
{
    ...
    if ((tileTransitionState == T_TILE_CENTER && forcedMove == FALSE) || tileTransitionState == T_NOT_MOVING)
    {
        if (GetPlayerSpeed() != 4)
        {
			...
			if (newKeys & L_BUTTON)				//register lr
                input->pressedLButton = TRUE;
            if (newKeys & R_BUTTON)
                input->pressedRButton = TRUE;	//end section
        }
...
int ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    //if (input->pressedSelectButton && UseRegisteredKeyItemOnField() == TRUE)	//register lr
	if (input->pressedSelectButton && UseRegisteredKeyItemOnField(0))
        return TRUE;
    else if (input->pressedLButton && UseRegisteredKeyItemOnField(1))
        return TRUE;
    else if (input->pressedRButton && UseRegisteredKeyItemOnField(2))	//end section
        return TRUE;
