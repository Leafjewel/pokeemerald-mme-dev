...
//void BagMenu_ItemPrintCallback(u8 windowId, s32 itemIndex, u8 a);		tm shops
void BagMenu_ItemPrintCallback(u8 windowId, s32 itemIndex, u8 a, u8 listPos);
void ItemMenu_UseOutOfBattle(u8 taskId);
void ItemMenu_Toss(u8 taskId);
//void ItemMenu_Register(u8 taskId);	//register lr
...
static const struct MenuAction sItemMenuActions[] = {
    ...
    [ITEMMENUACTION_DESELECT] =     {gMenuText_Deselect, ItemMenu_Deselect}, //{gMenuText_Deselect, ItemMenu_Register}, ,register lr
    ...
    [ITEMMENUACTION_SELECT_BUTTON] = {sMenuText_Select, ItemMenu_RegisterSelect},	//added for register lr
    [ITEMMENUACTION_L_BUTTON]      = {sMenuText_L, ItemMenu_RegisterL},			//added for register lr
    [ITEMMENUACTION_R_BUTTON]      = {sMenuText_R, ItemMenu_RegisterR},			//added for register lr
    [ITEMMENUACTION_BY_NAME] =      {sMenuText_ByName, ItemMenu_SortByName},	//added for bag sorting
    [ITEMMENUACTION_BY_TYPE] =      {sMenuText_ByType, ItemMenu_SortByType},	//added for bag sorting
    [ITEMMENUACTION_BY_AMOUNT] =    {sMenuText_ByAmount, ItemMenu_SortByAmount},	//added for bag sorting
...
//static const u8 sRegisteredSelect_Gfx[] = INCBIN_U8("graphics/interface/select_button.4bpp");	//register lr
...
};	//note to self: make max pocket items 129

struct TempWallyStruct {
    struct ItemSlot bagPocket_Items[BAG_ITEMS_COUNT];
	struct ItemSlot bagPocket_Medicine[BAG_MEDICINE_COUNT];	//more bag pockets
...
static EWRAM_DATA bool8 sRegisterSubMenu = FALSE;	//register lr
...
bool8 SetupBagMenu(void)
{
    ...
    case 13:
		...
		BagMenu_DrawPocketIndicatorSquare(0, 0);	//more bag pockets
        BagMenu_DrawPocketIndicatorSquare(5, 0);	//more bag pockets
        BagMenu_DrawPocketIndicatorSquare(6, 0);	//more bag pockets
        BagMenu_DrawPocketIndicatorSquare(7, 0);	//more bag pockets
    ...
    switch (gBagPositionStruct.pocket)
    {
        case TMHM_POCKET:
            ...
            else
            {
                ...
                ConvertIntToDecimalStringN(gStringVar1, itemId - ITEM_TM01 + 1, STR_CONV_MODE_LEADING_ZEROS, 3);	//for expanded/100 TMs
...
//void BagMenu_ItemPrintCallback(u8 windowId, s32 itemIndex, u8 y)	tm shops
...
void PrintItemSoldAmount(int windowId, int numSold, int moneyEarned)
{
    ...
    PrintMoneyAmount(windowId, 32, 1, moneyEarned, 0); //increase money limit
}
    ...
    if (MenuHelpers_CallLinkSomething() != TRUE && !gPaletteFade.active)
    {
        switch (GetSwitchBagPocketDirection())
        {
            ...
            default:
				...
				else if (JOY_NEW(START_BUTTON))	//bag sorting
                {
...
void BagMenu_DrawPocketIndicatorSquare(u8 x, u8 is_current_bag)
{
    if (is_current_bag == 0)
		...
		FillBgTilemapBufferRect_Palette0(2, 0x1017, x + 4, 3, 1, 1);	//more bag pockets
    else
        //FillBgTilemapBufferRect_Palette0(2, 0x102B, x + 5, 3, 1, 1);
		FillBgTilemapBufferRect_Palette0(2, 0x102B, x + 4, 3, 1, 1);	//more bag pockets
    ScheduleBgCopyTilemapToVram(2);
}
            ...
            else
            {
                switch (gBagPositionStruct.pocket)
                {
                    ...
                    case BERRIES_POCKET:
						...
						break;
                    case MEDICINE_POCKET:						// more bag pockets edits start
						gBagMenu->contextMenuItemsPtr = gBagMenu->contextMenuItemsBuffer;
                        gBagMenu->contextMenuNumItems = ARRAY_COUNT(sContextMenuItems_ItemsPocket);
                        memcpy(&gBagMenu->contextMenuItemsBuffer, &sContextMenuItems_ItemsPocket, sizeof(sContextMenuItems_ItemsPocket));
                        break;
                    ...
                    case POWERUP_POCKET:
                        ...
                        gBagMenu->contextMenuNumItems = ARRAY_COUNT(sContextMenuItems_ItemsPocket);
                        memcpy(&gBagMenu->contextMenuItemsBuffer, &sContextMenuItems_ItemsPocket, sizeof(sContextMenuItems_ItemsPocket));
                        break;                					// more bag pockets edits end
                }
            }
    }
...
void ItemMenu_Cancel(u8 taskId)
{
	...
	sRegisterSubMenu = FALSE;	//register lr
    BagMenu_RemoveSomeWindow();
    BagMenu_PrintDescription(data[1]);
    ScheduleBgCopyTilemapToVram(0);
    ScheduleBgCopyTilemapToVram(1);
...
void DisplaySellItemPriceAndConfirm(u8 taskId)
{
    ...
    ConvertIntToDecimalStringN(gStringVar1, (ItemId_GetPrice(gSpecialVar_ItemId) / 2) * tItemCount, STR_CONV_MODE_LEFT_ALIGN, 7); //increase money limit
...
void PrepareBagForWallyTutorial(void)
{
    ...
    memcpy(sTempWallyBag->bagPocket_Medicine, gSaveBlock1Ptr->bagPocket_Medicine, sizeof(gSaveBlock1Ptr->bagPocket_Medicine)); //more bag pockets
	...
	ClearItemSlots(gSaveBlock1Ptr->bagPocket_Medicine, BAG_MEDICINE_COUNT);		//more bag pockets
...
void RestoreBagAfterWallyTutorial(void)
{
    ...
    memcpy(gSaveBlock1Ptr->bagPocket_Medicine, sTempWallyBag->bagPocket_Medicine, sizeof(sTempWallyBag->bagPocket_Medicine));	//more bag pockets
...
void DoWallyTutorialBagMenu(void)
{
    ...
    GoToBagMenu(ITEMMENULOCATION_WALLY, MEDICINE_POCKET, CB2_SetUpReshowBattleScreenAfterMenu2);	//more bag pockets
}
...
#undef tUsingRegisteredKeyItem		//end section
...
enum ItemSortType
{
	...
	ITEM_TYPE_FOSSIL,
	ITEM_TYPE_MAIL,
	ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	ITEM_TYPE_BERRY_STATUS_RECOVERY,
	ITEM_TYPE_BERRY_EVS,
	ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	ITEM_TYPE_BERRY_BATTLE_BOOST,
	ITEM_TYPE_BERRY_POKEBLOCKS,
};
...
static const u16 sItemsByType[ITEMS_COUNT] =
{
	...
	[ITEM_SILVER_BOTTLECAP] = ITEM_TYPE_STAT_BOOST_DRINK,
    [ITEM_GOLD_BOTTLECAP] = ITEM_TYPE_STAT_BOOST_DRINK,
...
//    [ITEM_X_SPECIAL] = ITEM_TYPE_BATTLE_ITEM,
	...
	[ITEM_LEPPA_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	[ITEM_ORAN_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	[ITEM_SITRUS_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	[ITEM_FIGY_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	[ITEM_WIKI_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	[ITEM_MAGO_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	[ITEM_AGUAV_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	[ITEM_IAPAPA_BERRY] = ITEM_TYPE_BERRY_HEALTH_RECOVERY,
	...
	[ITEM_POMEG_BERRY] = ITEM_TYPE_BERRY_EVS,
	[ITEM_KELPSY_BERRY] = ITEM_TYPE_BERRY_EVS,
	[ITEM_QUALOT_BERRY] = ITEM_TYPE_BERRY_EVS,
	[ITEM_HONDEW_BERRY] = ITEM_TYPE_BERRY_EVS,
	[ITEM_GREPA_BERRY] = ITEM_TYPE_BERRY_EVS,
	[ITEM_TAMATO_BERRY] = ITEM_TYPE_BERRY_EVS,

	[ITEM_OCCA_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_PASSHO_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_WACAN_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_RINDO_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_YACHE_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_CHOPLE_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_KEBIA_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_SHUCA_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_COBA_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_PAYAPA_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_TANGA_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_CHARTI_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_KASIB_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_HABAN_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_COLBUR_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_BABIRI_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_CHILAN_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,
	[ITEM_ROSELI_BERRY] = ITEM_TYPE_BERRY_DAMAGE_REDUCTION,

	[ITEM_LIECHI_BERRY] = ITEM_TYPE_BERRY_BATTLE_BOOST,
	[ITEM_GANLON_BERRY] = ITEM_TYPE_BERRY_BATTLE_BOOST,
	[ITEM_SALAC_BERRY] = ITEM_TYPE_BERRY_BATTLE_BOOST,
	[ITEM_PETAYA_BERRY] = ITEM_TYPE_BERRY_BATTLE_BOOST,
	[ITEM_APICOT_BERRY] = ITEM_TYPE_BERRY_BATTLE_BOOST,
	[ITEM_LANSAT_BERRY] = ITEM_TYPE_BERRY_BATTLE_BOOST,
	[ITEM_STARF_BERRY] = ITEM_TYPE_BERRY_BATTLE_BOOST,

	[ITEM_RAZZ_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_BLUK_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_NANAB_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_WEPEAR_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_PINAP_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_CORNN_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_MAGOST_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_RABUTA_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_NOMEL_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_SPELON_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_PAMTRE_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_WATMEL_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_DURIN_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
	[ITEM_BELUE_BERRY] = ITEM_TYPE_BERRY_POKEBLOCKS,
};
...
static void AddBagSortSubMenu(void)
{
    switch (gBagPositionStruct.pocket + 1)
    {
        ...
        //case POCKET_TM_HM:
            ...
            break;
        default:				//applies to Medicine, Battle Items, and Power Up pockets too, +Berries now
...
static void SortItemsInBag(u8 pocket, u8 type)
{
    ...
    case TMHM_POCKET:
        ...
        break;*/		//reordered and added new expanded bag pockets
